...
type FilterLister interface {
	...
	List(selector labels.Selector) (ret []*v1.Filter, err error)
...
// List lists all Filters in the indexer.
func (s *filterLister) List(selector labels.Selector) (ret []*v1.Filter, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1.Filter))
	})
	return ret, err
}
...
type FilterNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*v1.Filter, error)
...
// List lists all Filters in the indexer for a given namespace.
func (s filterNamespaceLister) List(selector labels.Selector) (ret []*v1.Filter, err error) {
...
// Get retrieves the Filter from the indexer for a given namespace and name.
func (s filterNamespaceLister) Get(name string) (*v1.Filter, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1.Resource("filter"), name)
	}
	return obj.(*v1.Filter), nil
}
