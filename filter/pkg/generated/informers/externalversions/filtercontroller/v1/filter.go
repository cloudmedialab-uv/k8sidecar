...
import (
	"context"
	filtercontrollerv1 "filter/pkg/apis/filtercontroller/v1"
	versioned "filter/pkg/generated/clientset/versioned"
	internalinterfaces "filter/pkg/generated/informers/externalversions/internalinterfaces"
	v1 "filter/pkg/generated/listers/filtercontroller/v1"
	time "time"
	...
	return cache.NewSharedIndexInformer(
		&cache.ListWatch{
			ListFunc: func(options metav1.ListOptions) (runtime.Object, error) {
				...
				return client.FiltercontrollerV1().Filters(namespace).List(context.TODO(), options)
			},
			WatchFunc: func(options metav1.ListOptions) (watch.Interface, error) {
				if tweakListOptions != nil {
					tweakListOptions(&options)
				}
				return client.FiltercontrollerV1().Filters(namespace).Watch(context.TODO(), options)
			},
		},
		&filtercontrollerv1.Filter{},
		resyncPeriod,
		indexers,
	)
}
...
func (f *filterInformer) Informer() cache.SharedIndexInformer {
	return f.factory.InformerFor(&filtercontrollerv1.Filter{}, f.defaultInformer)
}

func (f *filterInformer) Lister() v1.FilterLister {
	return v1.NewFilterLister(f.Informer().GetIndexer())
}
